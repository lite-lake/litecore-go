...
// BaseConfigProvider 提供配置查询的公共实现
// 配置数据在创建后不可变，因此可以安全地在多个 goroutine 之间共享使用
type BaseConfigProvider struct {
	configData map[string]any
}

// NewBaseConfigProvider 创建基础配置提供者
func NewBaseConfigProvider(data map[string]any) *BaseConfigProvider {
...
func (p *BaseConfigProvider) Get(key string) (any, error) {
	if key == "" {
...
func (p *BaseConfigProvider) parsePath(path string) ([]pathPart, error) {
	var parts []pathPart
...
func (p *BaseConfigProvider) navigatePath(data map[string]any, path string) (any, error) {
	...
	for i, part := range parts {
		...
		if !exists {
			return nil, fmt.Errorf("config key '%s' not found", path)
		}
...
func (p *BaseConfigProvider) getArrayElement(value any, key string, index int, path string) (any, error) {
	arr, ok := value.([]any)
	if !ok {
...
func (p *BaseConfigProvider) pathError(path string, part int, format string, args ...any) error {
	msg := fmt.Sprintf(format, args...)
	return fmt.Errorf("config path '%s': %s", path, msg)
}

// Has 检查配置项是否存在
func (p *BaseConfigProvider) Has(key string) bool {
	_, err := p.Get(key)
	return err == nil
}
