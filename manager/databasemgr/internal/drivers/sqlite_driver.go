...
func NewSQLiteManager(cfg *config.DatabaseConfig) (*SQLiteManager, error) {
	...
	mgr := &SQLiteManager{
		BaseManager:   NewBaseManager("sqlite-database"),
		config:        cfg,
		driver:        "sqlite",
	...
	// 打开数据库连接
	db, err := sql.Open("sqlite3", cfg.SQLiteConfig.DSN)
	if err != nil {
	...
	// 配置连接池
	poolConfig := cfg.SQLiteConfig.PoolConfig
	if poolConfig != nil {
...
// configurePool 配置连接池
func (m *SQLiteManager) configurePool(poolConfig *config.PoolConfig) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// DB 获取数据库连接
func (m *SQLiteManager) DB() *sql.DB {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.db
}
...
// Driver 获取数据库驱动类型
func (m *SQLiteManager) Driver() string {
	return m.driver
}
...
// BeginTx 开始事务
func (m *SQLiteManager) BeginTx(ctx context.Context, opts *sql.TxOptions) (*sql.Tx, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// Stats 获取数据库连接池统计信息
func (m *SQLiteManager) Stats() sql.DBStats {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// Health 检查管理器健康状态
func (m *SQLiteManager) Health() error {
...
// OnStart 在服务器启动时触发
func (m *SQLiteManager) OnStart() error {
	// 验证数据库连接
...
// OnStop 在服务器停止时触发
func (m *SQLiteManager) OnStop() error {
	return m.Close()
}

// Shutdown 关闭管理器
func (m *SQLiteManager) Shutdown(ctx context.Context) error {
...
// 编译时检查：确保 SQLiteManager 实现了 common.Manager 接口
var _ common.Manager = (*SQLiteManager)(nil)