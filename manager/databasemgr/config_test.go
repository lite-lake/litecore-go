...
func TestDatabaseConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "none driver without config",
		...
		{
			name: "sqlite driver with valid config",
			cfg: &DatabaseConfig{
				Driver: "sqlite",
		...
		{
			name: "postgresql driver with valid config",
			cfg: &DatabaseConfig{
				Driver: "postgresql",
		...
		{
			name: "mysql driver with valid config",
			cfg: &DatabaseConfig{
				Driver: "mysql",
...
func TestParseDatabaseConfigFromMap(t *testing.T) {
	tests := []struct {
		...
		{
			name: "parse mysql config with pool",
			cfg: map[string]any{
				"driver": "mysql",
		...
		{
			name: "parse observability config",
			cfg: map[string]any{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr && !tt.check(cfg) {
				t.Error("ParseDatabaseConfigFromMap() config check failed")
			}
		})
	}
}
...
func TestParsePoolConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr && !tt.check(cfg) {
				t.Error("parsePoolConfig() config check failed")
			}
		})
	}
}
