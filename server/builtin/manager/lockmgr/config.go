...
func ParseLockConfigFromMap(cfg map[string]any) (*LockConfig, error) {
	...
	if redisConfigMap, ok := cfg["redis_config"].(map[string]any); ok {
		redisConfig, err := parseRedisLockConfig(redisConfigMap)
		if err != nil {
	...
	if memoryConfigMap, ok := cfg["memory_config"].(map[string]any); ok {
		memoryConfig, err := parseMemoryLockConfig(memoryConfigMap)
		if err != nil {
...
// parseRedisLockConfig 解析 Redis 锁配置
func parseRedisLockConfig(cfg map[string]any) (*RedisLockConfig, error) {
...
// parseMemoryLockConfig 解析内存锁配置
func parseMemoryLockConfig(cfg map[string]any) (*MemoryLockConfig, error) {
