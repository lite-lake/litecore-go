...
func Get[T any](p IConfigManager, key string) (T, error) {
	...
	if err != nil {
		...
		return zero, fmt.Errorf("%w: configmgr key '%s' - expected %s, got %s", ErrTypeMismatch, key, expectedType, actualType)
	}
...
func GetWithDefault[T any](p IConfigManager, key string, defaultValue T) T {
	val, err := Get[T](p, key)
	if err != nil {
		return defaultValue
	}
