...
func TestBuild(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "memory driver with empty configmgr",
			driverType:   "memory",
			driverConfig: map[string]any{},
			wantErr:      false,
			checkManager: func(t *testing.T, mgr ICacheManager) {
...
func TestBuildWithConfigProvider(t *testing.T) {
	tests := []struct {
		...
		{
			name: "memory driver with invalid configmgr type",
			configProvider: &MockConfigProvider{
				data: map[string]any{
					"cache.driver":        "memory",
		...
		{
			name: "configmgr provider returns error",
			configProvider: &MockConfigProvider{
				err: errors.New("configmgr error"),
			},
			wantErr: true,
		},
		{
		...
		{
			name: "redis driver with invalid configmgr type",
			configProvider: &MockConfigProvider{
				data: map[string]any{
					"cache.driver":       "redis",
