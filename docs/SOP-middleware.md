...
func NewCorsMiddleware() ICorsMiddleware {
    return middlewarepkg.NewCorsMiddleware()
}
```
...
func InitMiddlewareContainer(serviceContainer *container.ServiceContainer) *container.MiddlewareContainer {
    ...
    container.RegisterMiddleware[middlewares.ICustomRateLimiterMiddleware](middlewareContainer, middlewares.NewCustomRateLimiterMiddleware(100, time.Minute))
...
func NewMyMiddleware() IMyMiddleware {
    return &myMiddleware{
        order: middleware.OrderContext,  // 使用预定义常量
    }
}
```
...
- [component/middleware](../component/middleware) - 系统中间件实现
