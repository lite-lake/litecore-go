...
| 🟢 Low | config/utils.go:9 | import顺序不规范，第三方库lancet应位于local依赖之前 | 调整import顺序为：stdlib → third-party (lancet) → local (github.com/lite-lake/litecore-go) |
| 🟢 Low | util/string/string.go:7 | import顺序不规范，第三方库lancet应位于local依赖之前 | 调整import顺序为：stdlib (strings, unicode) → third-party (lancet) → local (github.com/lite-lake/litecore-go) |
